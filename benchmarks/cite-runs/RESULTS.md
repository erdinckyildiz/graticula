...
**So the two suites together are about a minute of engine time**, plus a
half-minute of container start, against a server that is already up. That is the
...
suite `ogcapi-features-1.0`, and `iut=` in place of `wfs=`.
