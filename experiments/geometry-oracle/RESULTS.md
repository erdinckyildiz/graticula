# Q-20's oracle — two engines, ninety-six comparisons, one divergence
...
`st_relate` (DE-9IM) and `st_dwithin` are the query path's alone — no Esri relation
name maps to them — so they have no second answer to diverge from.
...
where GEOS and JTS are documented to be able to part company, so the fifteen cases
...
## Result: 84 of 96 agree, and the twelve that do not are all one thing
...
**Every case that can be expressed through the product agrees, on all six
predicates.** Including the ones chosen because they should be hardest:
...
- On every case expressible through the product, the two agree. Q-20's *how do we
  ...
  measured, across fifteen adversarial cases and six predicates.**
...
- The cases are now a test —
  ...
  15 cases × 6 predicates. Falsified by making `Touch` mean `Crosses` in the worker,
  which failed it on three cases. So an NTS or GEOS upgrade that moves an answer
  fails the build, which is the only way this agreement can be lost.

**Not settled, and named:**

- **Coordinate magnitude.** Everything here is within 0–25. Floating-point
  divergence grows with magnitude, and web-Mercator metres run to 2×10⁷. Not
  measured.
...
- **`st_relate` and `st_dwithin`** have one engine each, which means no divergence
  and also no oracle. A DE-9IM pattern is the most intricate predicate in either
  library and nothing checks ours against anything.
