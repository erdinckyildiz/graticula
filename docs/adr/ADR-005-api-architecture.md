...
2. **The capability report is generated, never hand-maintained.** *(~~**LIVE and UNMET, 2026-08-20.** `OgcNames.ConformsTo` is a hand-maintained array of five URIs. This is the condition's own failure mode: the list is what a validator checks, and nothing ties it to what the code actually does — a class removed in the code stays claimed in the array until somebody notices.~~ ADR-008 §2 makes the same demand for the ArcGIS capability report and it is tracked there.)*
   ...
   ***DISCHARGED 2026-09-09, and not by generating the list — by binding each claim to its own proof.*** **Generating it from the routes is the obvious reading and it is the wrong one.** A conformance class is not a route: `core` is six resources and a link graph, `geojson` is a media type, `crs` is a query parameter, a response header and a collection property that have to agree with each other. Nothing derives those from an endpoint table, and a generator that tried would be **a second description of the specification**, which is [D-130](../architecture-debt.md)'s shape rather than a cure for it.

   **What the condition actually asked for is the sentence it ends on** — *nothing ties it to what the code actually does* — and that is now `EveryConformanceClaimIsProvenTests`, which **asserts in both directions**. A URI in the served list with no case in that file fails, so a class cannot be claimed until somebody writes what makes it true; a case whose probe fails also fails, so a class that stops being true stops being claimed. **The list is read from `/conformance` over HTTP rather than from the constant**, because a test that read the constant would pass against a server shipping a different one.

   **The five were measured before the test was written, which is the order that mattered**: `core` — the landing page carries `self`, `conformance`, `data` and `service-desc`, and items answer; `oas30` — `/api` answers `application/vnd.oai.openapi+json;version=3.0`, and the **version** is asserted, because a server answering Swagger 2 there would be claiming a class it does not meet while serving a document that parses; `geojson` — items answer `application/geo+json` and not `application/json`; `html` — `?f=html` answers `text/html`; `crs` — a collection advertises three CRSs and a `storageCrs`, and a request naming one comes back with `Content-Crs` naming it, which is the assertion rather than the status code, because a server that ignored the parameter would return storage coordinates with a 200. **The list was honest; what it lacked was anything that would notice if it stopped being.** **Falsified** by removing `html` from the proof list while the server still claimed it — the failure names the URI and quotes this condition.

   *(One thing found on the way, and it is a test defect that reads exactly like a product one: `Content-Crs` is a content header by name and is not one .NET recognises, so `HttpResponseMessage` files it under the message headers. Asking only the content — where the name says to look — failed against a server that was sending it correctly.)*
