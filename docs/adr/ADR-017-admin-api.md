...
| 3 | `PUT /admin/certificates/serving` | Install the replacement — no restart | **The route is not built; the thing it does happens anyway.** [ADR-014](ADR-014-tls-and-certificates.md) condition 1 — rotation verified without a restart — was discharged 2026-08-27: the listener holds a `ServerCertificateSelector`, `CertificateReload` watches the certificate file, and a replacement takes effect on the next handshake with warm service contexts intact. **So the operator's action is *copy the file* rather than *call this route***, and step 1's `note` says exactly that. An upload endpoint is a larger decision — a new authorization story, a new disclosure surface, and validating a blob before it can replace a working certificate — and it stays open |
