...
| **One city's buildings, served three ways** | 2026-09-14, fixture, linux-arm64, `cities-bench.py`. Overture `2026-07-22.0` buildings in two boxes — New York's five boroughs with the edge of New Jersey (1,658,511) and Istanbul province (1,270,971) — written once as GeoParquet (176 and 119 MB, WKB and a declared bbox covering), copied into one `.duckdb` file (477 MB) and into MotherDuck. Medians of five, milliseconds, NYC / Istanbul: count all 26 / 22 GeoParquet, 21 / 23 file, 34 / 34 MotherDuck; a name filter over every row 29–35 on all three; a count in a 1 km box 107 / 98, 255 / 200, **77 / 79**; the same box's features in 3857 188 / 178, 307 / 121, 132 / 115; five tallest by `height` 65 / 42, 33 / 29, 50 / 48. Publish 24–32 ms each, extents computed. **The DuckDB file is the slowest to a box** — its table is not spatially ordered and core has no index to use, where the GeoParquet file's row groups are ordered and skipped by the covering column, and MotherDuck's is fastest from a VPS in Germany. One run, one box per city, no cold-cache figures; it says each kind serves a city at interactive speed, not which is faster in general. **A MotherDuck source opened hours earlier did not list the two tables created since** until a later run; `refresh databases` now runs before each listing, and that this closes the gap is not measured |
