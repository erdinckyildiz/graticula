...
| Compatibility layer (§51) | **scope set, amended** | — | — | — | — | — | — | v1 is **WFS + WMTS-for-vector-tiles + full ArcGIS FeatureServer including edits + GeometryServer** (Q-17, Q-17a). **GeometryServer added 2026-08-13**: Q-17 excluded it as image-producing, which it is not — it is a REST wrapper over PROJ and NTS we already hold. GPServer out for a better reason (no toolbox). WMS, MapServer, ImageServer out (Q-47, ADR-004, ADR-009). Outside the core domain. |
