...
| | Detail |
|---|---|
| **Database** | **PostGIS only.** Both modes: hosted in our managed datastore (mandatory, Q-69) and **registered** in the customer's existing PostGIS, read/write where rights allow. **2026-08-14: hosted now means uploaded** — `POST /admin/hosted` takes a GeoJSON file, infers a schema, creates a table, reprojects to 3857 once and publishes a feature service and a tile service from it. Shapefile is [Q-98](open-questions.md) |
