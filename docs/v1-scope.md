...
| **The rest of the FeatureServer data model** | **Added 2026-09-09 by owner decision** — [Q-58c](open-questions.md), recorded in [ADR-013](adr/ADR-013-feature-service-data-model.md) §5a. **Domains, subtypes and editor tracking**, which ADR-013 §1 had left in Q-58 and not decided. They are missing in two different ways and the 2026-08-27 inventory is what says which: `domain` is *present and always null* on every field of every layer document, while `subtypeField`, `subtypes`, `types`, `templates` and `editFieldsInfo` are **absent from the document entirely**. **Editor tracking is a repair rather than a feature** — it is the only reason [D-20](architecture-debt.md) exists, where `features:edit` is narrower here than in ArcGIS Portal because *change your own* is unenforceable while the server cannot tell whose feature is whose. Two conditions carry the risk: a domain or subtype the server reports is one it **enforces on write**, and editor tracking discharges by D-20 closing rather than by four columns existing. Like relationships and attachments before them, this **enlarges the first release rather than following it**, and that is recorded rather than absorbed. **Editor tracking BUILT 2026-09-11 — [ADR-064](adr/ADR-064-editor-tracking-and-what-features-edit-means.md) — and D-20 closed with it**: a column given a role through the field overrides, `editFieldsInfo` and `ownershipBasedAccessControlForFeatures` on the layer document, and `features:edit` changing its own features on a tracked layer. ~~**Domains and subtypes are still NOT BUILT.**~~ **Domains and subtypes BUILT 2026-09-12 — [ADR-065](adr/ADR-065-domains-and-subtypes.md) — and ADR-013 condition 5 discharged with them**: a list or a range per column and a layer's subtypes, carried by the field overrides, reported in the layer document as `domain`, `typeIdField`, `subtypeField`, `types` and `subtypes`, refused outside by the writer both editing faces share, set from the Fields page, and carried in from a geodatabase. What is not measured is what an ArcGIS client does with them and an Esri-written archive's subtypes — ADR-065 conditions 2 and 3. ~~**NOT BUILT, measured 2026-09-10**~~ — `subtypeField` and `CodedValue` appear nowhere in `src`, `subtypes` appears once in a comment naming this row, and `editFieldsInfo` appears once as a null on the **ImageServer** document, which is a different surface. So the 2026-08-27 inventory still describes the product exactly, four weeks and one scope decision later *(true on 2026-09-10; since 2026-09-11 `editFieldsInfo` is on the FeatureServer layer document of every tracked layer, and the rest of that sentence still holds)* |
...
| **GeoParquet layers, read in place** | **Added 2026-09-13 on the owner's request** — *"artık duckdb ve geoparquet üzerinde çalışabiliriz … Sen o ilişkiyi kurar mısın?"* — [ADR-066](adr/ADR-066-geoparquet-layers-read-by-duckdb.md). A folder of GeoParquet files inside `Graticula:GeoParquetRoot` is registered like a database, and each file is published as a **read-only** layer that DuckDB reads in place, in the serving process, with its spatial extension never loaded. It answers the FeatureServer query, the layer document, counts, ids, extents, statistics, OGC API Features, WFS and WMS; it refuses editing, attachments, relationships, vector tiles, and the spatial relations other than intersects. **The request is the owner's; serving in place rather than converting into the datastore is `INFERRED`** — ADR-066 §11 lists what waits on confirmation. **§3a's deferral of DuckDB is lifted for this and nothing else**: DuckDB is not a second database a customer registers, and the other four engines stay where §3a put them. **Amended the same day by owner decision — [ADR-067](adr/ADR-067-duckdb-sources-beyond-a-local-folder.md)**: *"üçünü de destekleyelim"* — GeoParquet over `https://` and in an `s3://` prefix, a DuckDB database file, and a MotherDuck database, all read-only and each read by its own confined DuckDB; all three built the same day. With the last two, *DuckDB is not a second database a customer registers* stops being true, and this cell says so rather than keeping the sentence |
| **Feature history** | **Added 2026-09-19 by owner decision**, after the NextGIS Web comparison ([research/nextgis-web-comparison.md](research/nextgis-web-comparison.md)) named it as one of two things a user arriving from NextGIS Web or ArcGIS would miss: *"2. Kapsama alalım"*. [ADR-078](adr/ADR-078-a-hosted-layer-can-keep-its-history.md). **Hosted layers only**, switched on per layer by its owner: a trigger writes every version into `<table>__history`, so an edit made in QGIS or SQL is kept beside one made through this server, each with who made it. Read three ways — ArcGIS `historicMoment` on `query` (with `isDataArchived` on the layer document), the admin API, and a History page in Studio that puts any version back. `historicMoment` was on the ignored list until now, which answered a question about the past with the present; it is refused on a layer that keeps no history. **Opt-in, hosted-only, off-deletes-it and restore-as-an-edit are `INFERRED`** — [Q-155](open-questions.md). Measured: about 1.4× on an update, nothing measurable on an add or a delete ([benchmarks/feature-history](../benchmarks/feature-history/RESULTS.md)) |
| **Web maps** | **Added 2026-09-19 by owner decision**, the same sentence as feature history. [ADR-079](adr/ADR-079-a-web-map-is-a-saved-document.md). A map a user makes from several services, filters, measures on and saves — stored as an **ArcGIS Web Map document** and listed by the portal as a `Web Map` item, so ArcGIS clients open it too — with a viewer at `/studio/webmap.html` whose layout follows NextGIS Web's web map. It is [ADR-056](adr/ADR-056-an-item-is-its-own-thing-and-a-service-is-one-kind.md)'s first item with no service behind it; ADR-056's item table is still not written, and ADR-079 records that as its cost. **The format, its own table, three sharing scopes and the layout are `INFERRED`** — [Q-156](open-questions.md). `view.html` stays, as the operator's look at one service |
...
> **AMENDED 2026-09-13 — DuckDB returns, for one job.** The owner asked to work on DuckDB and
> GeoParquet, and [ADR-066](adr/ADR-066-geoparquet-layers-read-by-duckdb.md) builds it as the file-format query engine
> [Q-81](open-questions.md) described: read-only GeoParquet layers, read in place. **It is not a
> second database in the sense this section defers** — nobody registers a DuckDB, and nothing is
> written to one — so Oracle, SQL Server, MySQL and MariaDB stay deferred and the bullets below
> stay dormant for them. **Two of the bullets woke with it, and each brought its guard.** *Q-20
> dies* was already corrected to *two engines*; a GeoParquet layer's `intersects` is a third, and it
> is checked against PostGIS rather than trusted. *There is nothing to force* about a second dialect
> was already corrected above; the forcing function fired, and [D-162](architecture-debt.md)'s seam
> — the parsed tree travelling with the where clause — is what it produced.
