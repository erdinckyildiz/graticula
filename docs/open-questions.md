...
| Q-41 | Do we offer an optional companion schema in a registered database where granted rights, to hold bookkeeping? That is how ArcGIS gives referenced data advanced capability. Lower priority now that editing is out of the API. | Council | [research/arcgis-datastore-model.md](research/arcgis-datastore-model.md) §4 |
...
| Q-42 | Is feature editing through our API in scope? | **No, on the current reading.** Editing is done against the database with QGIS — including against our own datastore, which is an ordinary spatial database QGIS can connect to. One editing story, not two. If this reading is wrong, §28's CRUD, batch editing, editor tracking and optimistic concurrency all return. | 2026-08-12 | [data-model.md](data-model.md) §5 |
