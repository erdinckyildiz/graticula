...
| Q-40 | **[OWNER]** Do we accept data uploads at all, or does the administrator load data into their own database with QGIS and then register it? If the latter, the managed store disappears from v1 and the architecture gets materially smaller. | Owner | [data-model.md](data-model.md) §3 |
| Q-41 | Do we offer an optional companion schema in a registered database where granted rights, to hold versioning and editor-tracking bookkeeping? That is how ArcGIS gives referenced data advanced capability. It means being a resident in someone else's database. | Council | [research/arcgis-datastore-model.md](research/arcgis-datastore-model.md) §4 |
| Q-42 | **[OWNER]** Is feature editing through our API in scope at all? Asked and not yet answered. It changes §28 substantially, and it decides whether Q-41 matters. | Owner | ADR-008, §28 |
...
| Q-31 | Does the feature service expose provider capability differences, or hide them? | ~~Hosted gets full capability, registered gets provider capability.~~ **Withdrawn same day** — that rested on a misreading of the ArcGIS model, where referenced data is editable and some advanced capabilities *require* referencing. Reopened; see the blocking table. | withdrawn 2026-08-12 | [research/arcgis-datastore-model.md](research/arcgis-datastore-model.md) §1 |
| Q-38 | Can a layer move between registered and hosted, keeping its identity? | **No.** Editing is done in the source database directly, with QGIS. This narrowing is what makes the simplified three-role storage model available. | 2026-08-12 | [data-model.md](data-model.md) §3 |
