...
| Q-38 | Can a layer move between registered and hosted while keeping its service identity and URL? An administrator who registers a layer and later wants editing will ask for exactly this. Stable IDs (§37) should make it possible, but it must be designed rather than discovered. | Council | [data-model.md](data-model.md) §5, §37, §38 |
