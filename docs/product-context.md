...
| Licensing | Open source, copyleft (GPL/AGPL) acceptable — **note this was always about *inbound* dependency licences. The project's own outbound licence is Q-73 and is still unset; there is no `LICENSE` file.** | No dependency is excluded on licence grounds. LGPL/MIT are free of friction. AGPL obligations apply over the network for a server product and must be stated explicitly wherever relevant. |
