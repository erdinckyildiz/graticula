...
| **Why it exists (Q-49)** | **"I will give this to the world."** ~~Open source, public, unrestricted.~~ **Source-available, public, and restricted in exactly one way — corrected 2026-09-09, fifteen days late.** [ADR-047](adr/ADR-047-the-outbound-licence-is-elastic-2.md) made the outbound licence the Elastic License 2.0 on 2026-08-25: anyone may read, run, modify, distribute and charge money for it, and nobody may offer it to third parties as a hosted service. **The gift survives the correction and the words did not.** Sufficient on its own; a gift owes no market case. Dissolves §81's requirement to test Q-49 with real GIS teams. | Answered 2026-08-13 — see [competitive-position.md](competitive-position.md) §6 |
...
| **Licence** | ~~**Apache-2.0** (Q-73). Permissive with an express patent grant. Anyone may fork, close and sell it — accepted, because that is what *give it to the world* means. **Consequence: GPL and AGPL dependencies are now disqualified**, since we cannot sublicense them under Apache-2.0. The earlier *copyleft acceptable* note was about inbound dependencies and is superseded by this.~~ **Reversed 2026-08-25 by owner decision — [ADR-047](adr/ADR-047-the-outbound-licence-is-elastic-2.md) — and corrected here 2026-09-09, which is fifteen days in the document [CLAUDE.md](../CLAUDE.md) §7 sends a reader to for the decisions already taken.** The outbound licence is the **Elastic License 2.0** and this project is **source-available, not open source**. **The struck text is not merely out of date — it reasoned *from* the old licence**, which is the propagation shape [D-130](architecture-debt.md) records: its dependency conclusion followed from *we cannot sublicense them under Apache-2.0*, a premise that no longer holds. What replaces it is measured rather than derived: LGPL and MIT dependencies are unproblematic and **nothing linked here is copyleft** (ADR-047 §4), with obligations tracked in [DEPENDENCY-LICENSES.md](../DEPENDENCY-LICENSES.md). | No dependency is excluded on licence grounds. LGPL/MIT are free of friction. AGPL obligations apply over the network for a server product and must be stated explicitly wherever relevant. |
...
| **Compatibility surface** | ~~WFS, WMTS-for-vector-tiles, full FeatureServer; not GeometryServer or GPServer~~ → **superseded by Q-88.** v1 is **ArcGIS FeatureServer, VectorTileServer and GeometryServer**, and they are the *primary* surface rather than a compatibility layer. WFS and WMTS move to v2 with OGC API Features. **Amended 2026-08-19: WFS comes back first, and not beside OGC API Features.** [ADR-039](adr/ADR-039-wfs-is-the-first-surface-after-v1.md) makes WFS 2.0 read-only the first surface after v1, ahead of the native API this row pairs it with — so the v2 order is WFS, then the rest, and the *compatibility* surface is once more the thing built before the thing it is compatible with. v1 itself is unchanged. | Answered 2026-08-12, superseded 2026-08-13 — [v1-scope.md](v1-scope.md) |
| **Who it is for, now (2026-09-10)** | **A showcase — for people to look at.** Asked whether a real deployment was coming: it is not. The product is evaluated by strangers on GitHub and used by its owner. **This reorders the open work rather than adding any.** [D-249](architecture-debt.md) — the request pipeline saturating at sixteen concurrent callers, about six requests a second — is a real defect that **nobody evaluating this will reach**, and [ADR-002](adr/ADR-002-primary-data-architecture.md) condition 3's export/import format reproduces a deployment nobody else has. What matters instead is **the path a stranger walks**: the GitHub pages, the quickstart, a container that starts, a console that opens, a service published, that service opened in Pro or QGIS. A defect on that path outranks a defect anywhere else, whatever its size. | Owner decision, 2026-09-10 |
| **v1's finish line** | **"Şu an v1 için gayet yeterli bir durumdayız."** v1 is **not pending work**. The 47 live ADR conditions were being read as a checklist gating a release and they are not one: they are **a defect source and a record**. **The consequence is that the count stops being a target** — closing a condition is worth doing when it finds something or corrects the record, and worth nothing when it only moves a number. Today's three closes each corrected a document that had been wrong for days; that is the test. | Owner decision, 2026-09-10 |
| **How much is written down** | **This much. Keep going.** Asked whether the ADR and debt-row prose — measurement, reasoning, falsification, and the naming of what was *not* done — is too long: it is the right level. **So the record keeps the parts that are cheap to drop and expensive to lose**: how a thing was measured, what was tried and abandoned, and what a repair deliberately did not cover. | Owner decision, 2026-09-10 |
| **How the work is paced** | **Long uninterrupted blocks, reported at the end.** Not short rounds with frequent check-ins. **The cost is accepted and named**: on 2026-09-10 a defect was diagnosed wrongly twice inside one block and the wrong diagnosis was committed before the right one arrived — a checkpoint would have caught it sooner. The answer is not to interrupt more; it is to measure earlier, which is what [read-the-failing-run-s-own-log](#) became. | Owner decision, 2026-09-10 |
| **Versioning and releases** | **`1.0.0` from 2026-09-10, and the patch advances on every commit.** *"artık v1.0.0'a geç ve her commit'te sonraki rakamı 1 artır."* A commit is a release: it is tagged `v1.0.z`, the release workflow runs the README's four commands against a built-from-source stack before it publishes anything, and both images go to GHCR under that number and under `latest`. **What it buys is [D-256](architecture-debt.md)**: `latest` had drifted 247 commits and four schema migrations behind the README, so a reader following the quickstart got **404** from the publish endpoint the same page documents. **What it costs is said in [SECURITY.md](../SECURITY.md) rather than hidden**: a pinned `1.0.7` is a pinned commit, not a milestone anybody paused over, and no older patch is a maintained line — the fix is always in the newest. **The banner carries `v1` and not a patch**, because a poster re-rendered on every push is a poster nobody re-renders. | Owner decision, 2026-09-10 |
