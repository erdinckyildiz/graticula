...
|---|---|---|---|
| **B1** — **`REOPENED` 2026-08-13** | **Contradiction sweep (§63)** — round 1 ran and discharged this blocker; independent review 3 found that **all eleven of its findings were forward-facing** (scope added without propagating) and **zero were backward-facing** (premises deleted without propagating), which is the larger and more mechanical class. See [reviews/independent-review-3-synthesis.md](reviews/independent-review-3-synthesis.md) §2a. Original entry: | Three owner decisions reversed within hours of each other on 2026-08-12, one of them reversing a headline decision taken the same morning. Something has been missed — **and on 2026-08-13 the owner found the first one by pointing at a question number**: Q-17 excluded four ArcGIS service types on the grounds that they *produce rendered images*, which is false for two of them. One bundled sentence carried four decisions and inherited the weakest justification in the bundle. If one glance found that, a sweep will find more | ~1 session, **and now demonstrably worth it** |
| ~~**B2a**~~ | ~~ADR-003, single-provider half~~ **DONE 2026-08-13 — `ACCEPTED WITH CONDITIONS`** | Where geometry work happens on the hot path. **Already answered by benchmark runs 1–3**: clip and simplify are ours, topology stays NTS, and the boundary is empirically placed rather than argued. Needs writing up as a decision, not investigating | ~1 session |
...
| ~~**B5**~~ | ~~Packaging (Q-71)~~ **DONE 2026-08-13 — [ADR-016](adr/ADR-016-packaging-deployment-upgrade.md)** | The baseline is now three images — server, mandatory datastore, job worker with GDAL — with no story for how they are packaged, version-matched, upgraded or carried into an air-gapped install. "Run it" has no answer | ~1 session |
